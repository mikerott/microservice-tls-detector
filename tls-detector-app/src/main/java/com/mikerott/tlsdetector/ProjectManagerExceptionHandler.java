...
import com.mikerott.tlsdetector.responsemodel.ErrorObjectResponse;
import com.mikerott.tlsdetector.responsemodel.ErrorResponse;
...
public class ProjectManagerExceptionHandler {
	...
	public @ResponseBody ResponseEntity<ErrorResponse> methodArgumentNotValidExceptionHandler(
		...
		for (ObjectError objectError : objectErrors) {
			...
			errorObjects.add(new ErrorObject(objectError.getCode() + ":" + message.toString(), logger));
	...
	public @ResponseBody ResponseEntity<ErrorResponse> defaultErrorHandler(final Exception e,
			final WebRequest request) {
		ErrorObject errorObject;
		// This is truly an unhandled exception by virtue of us ending up in the
		// defaultErrorHandler. We should log it:
	...
	private ResponseEntity<ErrorResponse> handleExceptionInternal(Exception ex, List<ErrorObject> errors,
	...
	private ErrorObject createErrorObject(Exception e, HttpStatus defaultCode) {
		return new ErrorObject(String.format(DEFAULT_ERROR_MESSAGE, e == null ? "unknown" : e.getClass().getName(),
				e == null ? "unknown" : e.getMessage()), logger);
	}
